...
public class InlineQuerySystem : ComponentSystem, IAnonymousWorldObject {
    public InlineQuerySystem(World world, string name, QueryAction action, Action<QueryAction> onExecute = null) :
        base(world) {
        this.Name = name;
...
public class InlineSystem<TState> : ComponentSystem, IAnonymousWorldObject {
    public InlineSystem(World world, string name, Func<TState> onStart, Action<InlineSystem<TState>> onExecute,
        Action<TState> onDispose = null) : base(world) {
