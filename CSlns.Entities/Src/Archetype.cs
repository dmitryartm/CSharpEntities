...
namespace CSlns.Entities {
    public class Archetype {
        ...
        public string Id {
            get {
                if (this.components.IsEmpty && this.sharedComponents.IsEmpty) {
                ...
                if (this._id == null) {
                    ...
                    id.AppendLine();
                    foreach (var sharedComponent in this.sharedComponents.Values) {
        ...
        public bool HasSharedComponent(Type type) {
            return this.sharedComponentIndexer.TryIndex(type, out var index) && this.sharedComponents.ContainsKey(index);
        }

        public bool HasSharedComponent<T>()  where T : ISharedComponent<T> {
            return this.HasSharedComponent(typeof(T));
        }

        public bool HasSharedComponent<T>(T value) where T : ISharedComponent<T> {
            return this.sharedComponentIndexer.TryIndex(value, out var index) && this.sharedComponents.Contains(new KeyValuePair<int, SharedComponentIndex>(index.TypeIndex, index));
        }

        public bool TryGetSharedComponent<T>(out T value) where T : ISharedComponent<T> {
            if (this.sharedComponentIndexer.TryIndex<T>(out var typeIndex)
                && this.sharedComponents.TryGetValue(typeIndex, out var sharedComponentIndex)) {
                value = this.sharedComponentIndexer.Value<T>(sharedComponentIndex);
                return true;
            }
            value = default;
            return false;
        ...
        public Archetype AddComponents(params Type[] components) {
            return this.AddComponents((IEnumerable<Type>)components);
        }

        public Archetype AddComponents<T0>() {
            return this.AddComponents(typeof(T0));
        }

        public Archetype AddComponents<T0, T1>() {
            return this.AddComponents(typeof(T0), typeof(T1));
        }

        public Archetype AddComponents<T0, T1, T2>() {
            return this.AddComponents(typeof(T0), typeof(T1), typeof(T2));
        }

        public Archetype AddComponents<T0, T1, T2, T3>() {
            return this.AddComponents(typeof(T0), typeof(T1), typeof(T2), typeof(T3));
        }

        public Archetype AddComponents<T0, T1, T2, T3, T4>() {
            return this.AddComponents(typeof(T0), typeof(T1), typeof(T2), typeof(T3), typeof(T4));
        }

        public Archetype AddComponents<T0, T1, T2, T3, T4, T5>() {
            return this.AddComponents(typeof(T0), typeof(T1), typeof(T2), typeof(T3), typeof(T4), typeof(T5));
        }

        public Archetype AddSharedComponent<T>(T value) where T : struct, ISharedComponent<T> {
            var index = this.sharedComponentIndexer.Index(value);
            return new Archetype(
                this.componentTypeIndexer,
                this.sharedComponentIndexer,
                this.components,
        ...
        public override string ToString() {
            ...
            foreach (var sharedComponent in this.SharedComponents) {
